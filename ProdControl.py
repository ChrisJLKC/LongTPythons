...
class Control:
    ...
    def Checkup(self):
        """
        Collect Diagnostics and current state of other objects. Write diagnostic data to Database.
        """
        # [Sensor, Pump]
    ...
    def UpdateSchedule(self):
        """
        Update the schedule based on what is already scheduled, state of other objects and collected Diagnostics
        """
        
        #Schedule Pump if not currently scheduled and moisture is too low
        if not self.Schedule.isScheduled(self.Pump_Control.start_pump, self.Pump_Control.stop_pump):
            if self.InternalData[0][0] < self.min_moisture:
                self.Schedule.add( (self.Pump_Control.start_pump, None, (0, 0, 1)) )
                self.Schedule.add( (self.Pump_Control.stop_pump, None, (0, 0, 3)) )
            else:
                pass
    ...
    def ExecuteNextTask(self):
        """
        Encapsulated way to execute next task without needing to go through control to scheduler to execute.
        """
